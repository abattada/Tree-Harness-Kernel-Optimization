...
\begin{figure*}[t]
  \centering
  \resizebox{0.95\textwidth}{!}{\input{make_pipeline_fig.tex}}
...
\begin{figure*}[t]
  \centering
  \resizebox{0.96\textwidth}{!}{\input{make_harness_fig.tex}}
...
\texttt{swiglu}, not otherwise plotted---are in Appendix~\ref{app:allops}.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figures/budget_curves.png}
  \caption{Per-iteration optimization progress (DeepSeek API loop): best-so-far
  speedup vs.\ candidates evaluated, for the focus operators whose search visibly
  improves over iterations (flat-from-first-success operators omitted).
  \texttt{cross\_entropy} is repaired at candidate~2; \texttt{rope} climbs via a
  structural improvement around candidate~8.}
  \label{fig:budget}
\end{figure}
...
materialized. On plain \texttt{gemm} and \texttt{addmm} no engine beats cuBLAS:
the API loop produces 0/12 correct for both, while the agent merely matches it
($1.01\times$), as expected for already-optimal fp16 GEMM. The hard kernels reward
the agent's compile--read--repair loop.
...
reaches $5\times$ on \texttt{flce}, $1.09\times$ on \texttt{int4\_gemm}, and matches
cuBLAS on \texttt{gemm} and \texttt{addmm} ($\approx$1.0$\times$), where the API
loop produced no correct kernel at all.
...
\begin{table}[t]
  ...
  \begin{tabular}{lcccc}
    ...
    \texttt{addmm}      & 0/12 & 0/12 & 5/6\,@1.01 & 1.01$\times$ (B) \\
    \bottomrule
  \end{tabular}
\end{table}
...
correctness is then easy and the speedup follows. The budget curves
(Fig.~\ref{fig:budget}) place most of the gain in the first few candidates, which
argues for spending budget on seed diversity and fast repair rather than deep
refinement. The control group
...
\label{app:allops}
All 17 operators in the suite were run through the harness, and all 17 reached a
correct kernel. Table~\ref{tab:allops} gives the best valid speedup for each and
the engine that achieved it. The dense matmul kernels (\texttt{gemm},
\texttt{addmm}) only match cuBLAS ($\approx$1.0$\times$), as expected for an
already-optimal fp16 GEMM, and both were solved by the agent. Memory-bound figures
are the \textsc{DeepSeek}-flash values (clean, exclusive-GPU timing).
...
\begin{table}[h]
  ...
  \begin{tabular}{llrl}
    ...
    \texttt{swiglu}        & elementwise   & 1.69$\times$ & DeepSeek \\
    ...
    \texttt{addmm}         & matmul        & 1.01$\times$ & Claude agent \\
    \bottomrule
  \end{tabular}
\end{table}
